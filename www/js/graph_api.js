...
function displayCheckIns(checkins, dom) {
  var i,
    checkin,
    markup = '<div class="data-header">Your last five check-ins:</div>';

  for (i = 0; i < checkins.length && i < 5; i++) {
    checkin = checkins[i];

    markup += '<div class="place">' + '<div class="picture"><img src="http://graph.facebook.com/' + checkin.place.id + '/picture"></div>' + '<div class="info">' + '  <div class="name">' + checkin.place.name + '</div>' + '  <div class="check-in-msg">' + (checkin.message || '') + '</div>' + '</div>' + '</div>';
  }
...
function displayPlaces(places, dom) {
  var i,
    place,
    markup = '<div class="data-header">Nearby locations:</div>';

  for (i = 0; i < places.length && i < 5; i++) {
    place = places[i];

    markup += '<div class="place">' + '<div class="picture"><img src="http://graph.facebook.com/' + place.id + '/picture"></div>' + '<div class="info">' + '  <div class="name">' + place.name + '</div>' + '  <div class="check-in-button"><input type="button" value="Check in" onclick="checkin(' + place.id + ')" /></div>' + '</div>' + '</div>';
  }
...
function checkin(id) {
  ...
  FB.api('/me/checkins', params, function (response) {
    clearAction();
